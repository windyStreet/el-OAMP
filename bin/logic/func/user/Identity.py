...
# 导入常量并重命名
from bin.logic.func.user import constant as const
...
class IdNumber(object):
    ...
    @classmethod
    def generate_id(cls, sex=0, bron_range_start="1940-01-01", bron_range_end=None):
        if bron_range_start is None:
            bron_range_start = (datetime.datetime.now() - datetime.timedelta(days=365 * 100)).strftime('%Y-%m-%d')

        if bron_range_end is None:
            bron_range_end = datetime.datetime.now().strftime('%Y-%m-%d')

        '''随机生成身份证号，sex = 0表示女性，sex = 1表示男性'''
        # 随机生成一个区域码(6位数)
        id_number = str(random.choice(list(const.AREA_INFO.keys())))
        # 限定出生日期范围(8位数)
        start, end = datetime.datetime.strptime(bron_range_start, '%Y-%m-%d'), datetime.datetime.strptime(bron_range_end, '%Y-%m-%d')
        birth_days = datetime.datetime.strftime(start + datetime.timedelta(random.randint(0, (end - start).days + 1)), '%Y%m%d')
...
#     print(IdNumber.verify_id('421302199209128478'))  # 检验身份证是否正确:False


def getInstance(id_number=None):
    return IdNumber(id_number)
